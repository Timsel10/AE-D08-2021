...
def transformation_matrix_gen():
    ...
    d_init =                # origin body reference <-> head reference
...
#def position_transform_inertial_to_head():
    ### position
...
#def velocity_transform_inertial_to_head():
    ### veloctiy
    ...
    vec_v_hr = np.array([-vec_v_br[1],[-vec_v_br[2]],[vec_v_br[0]]) + 

    return

#def acceleration_transform_inertial_to_head():
   ### acceleration
   # acceleration vector inertial frame
   vec_a_in = np.array([cx_ddot_in],[cy_ddot_in],[cz_ddot])

   # acceleration vector body frame
   vec_a_br = np.dot([trans_matrix, vec_a_in])

   # acceleration vector head frame
   vec_a_hr = np.array([-vec_a_br[1], [-vec_a_br[2], [vec_a_br[0]]) + 

   return



np.array([1.327*pd], [1.327*qd], [1.327*rd]) - 